...
const api = {
  ...
  delegateLogin: () => ipcRenderer.invoke('delegate:login'),
  delegateConnect: (path: string) => ipcRenderer.invoke('delegate:connect', path),
  delegateSetTier: (tier: 'free-only' | 'auto') => ipcRenderer.invoke('delegate:tier', tier),
  brainModels: () => ipcRenderer.invoke('brain:models'),
  brainSelectModel: (modelPath: string) => ipcRenderer.invoke('brain:selectModel', modelPath),
...
export type TadashiApi = {
  ...
  delegateLogin: () => Promise<{ opened: boolean; path: string }>;
  delegateConnect: (path: string) => Promise<{ ok: boolean; installed: boolean; authenticated: boolean; path: string; reason?: string }>;
  delegateSetTier: (tier: 'free-only' | 'auto') => Promise<{ delegateModelTier: 'free-only' | 'auto' }>;
  brainModels: () => Promise<{ models: string[]; current: string; path: string }>;
  brainSelectModel: (modelPath: string) => Promise<{ ok: boolean; loaded: boolean; cancelled?: boolean; model: string; path: string }>;
